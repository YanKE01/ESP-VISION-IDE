...
import stubs from '../build/stubs.json'

/*
 * esp-vision module completion (sensor / display / espdl)
 *
 * Data is generated by build.py from the device .pyi stubs. Completion is
 * offered after `module.` for the known modules, and after `var.` when `var`
 * was assigned from a known constructor (e.g. `lcd = display.Display(...)`).
 */

const STUB_MODULES = stubs.modules || {}
const STUB_CLASSES = stubs.classes || {}
const STUB_CTORS = stubs.ctors || {}
const STUB_CTOR_RE = /^[ \t]*([A-Za-z_]\w*)\s*=\s*([A-Za-z_]\w*\.[A-Za-z_]\w*)\s*\(/gm

function scanVarTypes(doc) {
    const types = {}
    let m
    STUB_CTOR_RE.lastIndex = 0
    const text = doc.toString()
    while ((m = STUB_CTOR_RE.exec(text))) {
        const cls = STUB_CTORS[m[2]]
        if (cls) {
            types[m[1]] = cls
        }
    }
...
function stubCompletionSource(context) {
    const dot = context.matchBefore(/[A-Za-z_]\w*\.\w*/)
    if (!dot) {
        return null
    }
    const parts = /^([A-Za-z_]\w*)\.(\w*)$/.exec(dot.text)
    if (!parts) {
        return null
    }
    const base = parts[1]
    const from = dot.from + base.length + 1
    if (STUB_MODULES[base]) {
        return { from, options: STUB_MODULES[base].members, validFor: /^\w*$/ }
    }
    const varType = scanVarTypes(context.state.doc)[base]
    if (varType && STUB_CLASSES[varType]) {
        return { from, options: STUB_CLASSES[varType], validFor: /^\w*$/ }
    }
    return null
}
...
export async function createNewEditor(editorElement, fn, content, options) {
    ...
    if (fn.endsWith('.py')) {
        ...
        mode = [
            ...
            indentUnit.of('    '), py,
            py.language.data.of({ autocomplete: stubCompletionSource }),
